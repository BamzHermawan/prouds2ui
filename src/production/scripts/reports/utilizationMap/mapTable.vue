	...
	methods: {
		...
		weekEndClass(day, isHeader = false) {
			...
			if (isHeader === false) {
				...
				hasWarning = day.isWarning;
				underutilize = day.underUtil;
			} else {
			...
			return {
				...
				"has-background-grey-lighter": weekend,
				"has-bg-success-lighter": !weekend && isHeader,
				"has-warning-approval": hasWarning,
				"is-underutilize": underutilize,
		...
		fetchMap() {
			if (this.unitID !== undefined) {
				let self = this;
				fetch
					.getUserMapByUnit(this.unitID, this.mapRange)
					.then(res => {
						let map = res.data;
